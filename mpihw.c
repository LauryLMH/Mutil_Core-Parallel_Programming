...
void serialPCC() {
	...
	* total of all values in a (used to calculate the mean in the next steep).
	*/
...
void parallelPCC() {
	...
	// Store the starting clock time
	if (rank == 0) {
		printf("start to process parllel function \n");

		begin_init = clock();
		printf("start to timing \n");
	}
	...
	*/
	...
	for (int i = 0; i < localSize; i++) {
		...
		local_b[i] = sin(i + 2 + offset);

		if (i < localSize - 1) {
			local_a[i + 2] = local_b[i];
		}
	...
	if (rank == comm_sz - 1) {
		...
		lastB2 = local_b[localSize - 2];
		MPI_Send(&lastB, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
		MPI_Send(&lastB2, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
	}
	...
	if (rank == 0) {
		...
		MPI_Recv(&lastB2, 1, MPI_DOUBLE, comm_sz - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		meanA = totalA / MAX_LENGTH;

		totalB = totalA + lastB + lastB2 - a1;
	...
	*/
	...
	if (rank == 0) {
		...
		double p2 = totalTempSum / (sdA * sdB);
		printf("\nPearson Correlation Coefficient: %.12f\n", p2);
